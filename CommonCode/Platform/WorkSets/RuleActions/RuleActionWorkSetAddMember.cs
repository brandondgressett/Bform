...
public class RuleActionWorkSetAddMember : IRuleActionEvaluator
{
    ...
    public RuleActionWorkSetAddMember(IApplicationAlert alerts, WorkSetLogic logic)
    {
        _alerts = alerts;
        _logic = logic;
    }

    public string Name => RuleUtil.FixActionName(nameof(RuleActionWorkSetAddMember));
        ...
        IEnumerable<string>? eventTags = null)
    {
        using (PerfTrack.Stopwatch(nameof(RuleActionCreateWorkSet)))
        {
            try
            {
                ...
                await _logic.EventAddMember(origin, workSet.Value, member.Value, null, eventTags, sealEvents, trx);
